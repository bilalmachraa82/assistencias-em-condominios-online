-- NOVA ESTRUTURA DE BASE DE DADOS - GESTÃO DE CONDOMÍNIOS
...
-- ========================================
-- FASE 2: ELIMINAÇÃO DA ESTRUTURA ANTIGA
-- ========================================
...
-- Remover functions
DROP FUNCTION IF EXISTS audit_security_event(text, text, bigint, text, text, jsonb);
DROP FUNCTION IF EXISTS audit_sensitive_operation(text, text, bigint, jsonb);
DROP FUNCTION IF EXISTS create_admin_user(text);
DROP FUNCTION IF EXISTS delete_assistance_safely(bigint);
DROP FUNCTION IF EXISTS generate_magic_code();
DROP FUNCTION IF EXISTS get_user_role(uuid);
DROP FUNCTION IF EXISTS log_assistance_deletion();
DROP FUNCTION IF EXISTS log_table_access();
DROP FUNCTION IF EXISTS trigger_set_timestamp();
DROP FUNCTION IF EXISTS update_assistance_by_token(bigint, text, text, timestamptz, text, text);
DROP FUNCTION IF EXISTS update_assistance_status(bigint, text, timestamptz);
DROP FUNCTION IF EXISTS validate_edge_function_access(text, text);
DROP FUNCTION IF EXISTS validate_magic_code(text);
DROP FUNCTION IF EXISTS validate_supplier_token(text, bigint, text);
DROP FUNCTION IF EXISTS validate_token_access(bigint, text, text);
DROP FUNCTION IF EXISTS validate_token_with_audit(bigint, text, text, text);
